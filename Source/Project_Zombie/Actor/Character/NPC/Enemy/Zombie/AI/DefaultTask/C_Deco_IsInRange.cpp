...
bool UC_Deco_IsInRange::CalculateRawConditionValue(UBehaviorTreeComponent& _OwnCom, uint8* _NodeMem) const
{
	...
	const float Dist = FVector::Dist2D(pZombie->GetActorLocation(), pTargetActor->GetActorLocation());
	...
	UC_Util::Print("SkillRange : " + FString::SanitizeFloat(SkillCom->GetSkillRange(m_SkillSlot)), FColor::Cyan, 10.f);
...
void UC_Deco_IsInRange::OnBecomeRelevant(UBehaviorTreeComponent& _OwnCom, uint8* _NodeMemory)
{
	Super::OnBecomeRelevant(_OwnCom, _NodeMemory);
}

void UC_Deco_IsInRange::OnCeaseRelevant(UBehaviorTreeComponent& _OwnCom, uint8* _NodeMemory)
{
	Super::OnCeaseRelevant(_OwnCom, _NodeMemory);
}
